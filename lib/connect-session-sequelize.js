...
module.exports = function SequelizeSessionInit(Store) {
  ...
  SequelizeStore.prototype.set = function setSession(sid, data, fn) {
    var self = this;
        ...
        if (!data || !data.passport || !data.passport.user) {
          debug("No user set by passport; will not create session.");
          return fn(null);
        }
        ...
        if (data.cookie && data.cookie.expires) {
          ...
          expires = new Date(Date.now() + self.options.expiration);
        }

        self.sessionModel.findOrCreate({
          where: {
            'sid': sid
          },
          defaults: {
          ...
          transaction: null
        }).spread(function sessionCreated(session, wasCreated) {
          if(session['data'] !== stringData) {
            session['data'] = JSON.stringify(data);
            session['expires'] = expires;
            session.save({ transaction: null }).then(function updated(session) {
              if (fn) {
                fn(null, data);
              }
  ...
  SequelizeStore.prototype.touch = function touchSession(sid, data, fn) {
    ...
    this.sessionModel.update({
      ...
      transaction: null
    }).spread(function sessionTouched() {
      if (fn) {
        fn(null);
      }
  ...
  SequelizeStore.prototype.destroy = function destroySession(sid, fn) {
    ...
    this.sessionModel.find({
      ...
      if (session === null) {
        ...
        session.destroy({ transaction: null }).then(function destroyedSession() {
  ...
  SequelizeStore.prototype.clearExpiredSessions = function clearExpiredSessions(fn) {
    ...
    this.sessionModel.destroy({
      ...
      transaction: null
    }).then(function sessionsDestroyed() {
      if (fn) {
        fn();
      }
    }).catch(function errorFindingSession(error) {
